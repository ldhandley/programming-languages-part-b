...
;Define (abstract) syntax of lang B with Racket structs
; -B is called MUPL in homework
;Write B programs directly in Racket via constructors
;Implement interpreter for B as a (recursive) Racket function (like our math eval one from before)

;Now, a subtle-but-important distinction:
; -Interpreter can assume input is a "legal AST for B"
;  -OK to give wrong answer or inscrutable error otherwise
...
;Interpreter returns expressions, but not any expressions
; -Result should always be a value (a kind of exp that evaluates to itself)
; -If not, interpreter has a bug.
...
;What if program is a legal AST, but evaluation of it tries to use the wrong kind of value?
...
;  -No need to check if any kind of value is OK
